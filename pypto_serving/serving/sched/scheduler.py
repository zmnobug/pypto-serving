...
class Scheduler:
    ...
    def _cache_unfinished_radix(self, request: Request) -> None:
        page_size = self.kv_cache_manager.block_size
        computed_kv_len = min(request.num_computed_tokens, request.num_tokens)
    ...
    def _should_defer_for_radix_producer(self, request: Request) -> bool:
        ...
        for producer in self.running:
            if producer.request_id == request.request_id:
                continue
            if producer.num_computed_tokens >= page_size:
                continue
    ...
    def _cache_request_prefix(self, request: Request) -> None:
        if self._use_radix_cache:
            self._cache_unfinished_radix(request)
        else:
